...
# first, let's remove the rows we don't want. In the trial_type column, it tells us what type of question/display we had. It saves information for all of them, but we are not always interested in all of them. This depends on each experiment but in your case, you are only interested in 'text_slider_response' and 'likert_survey' as that's where participants responses are stored. The rest can go. We will add in the audio information later as it's not randomised

data <- subset(data, task %in% c('text_slider_response', 'likert_survey'))
...
```{r formating}
# let's deal with the stimulus column first. Renaming.

data <- data %>%
  mutate(stimulus = case_when(
    stimulus == "<p>When I am sad, I listen to music.</p>" ~ "sad",
    stimulus == "<p>When I am happy, I listen to music.</p>" ~ "happy",
    stimulus == "<p>When I am angry, I listen to music.</p>" ~ "angry",
    stimulus == "" ~ NA,
